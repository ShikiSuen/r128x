...
public enum RF64Support {
    ...
    private static let RF64_CHUNK_ID: UInt32 = 0x34364652 // "RF64" in little-endian
    
    /// Standard WAV RIFF chunk identifier  
    private static let RIFF_CHUNK_ID: UInt32 = 0x46464952 // "RIFF" in little-endian
    
    /// ds64 chunk identifier (contains 64-bit size information)
    private static let DS64_CHUNK_ID: UInt32 = 0x34367364 // "ds64" in little-endian
    
    /// WAVE format identifier
    private static let WAVE_FORMAT_ID: UInt32 = 0x45564157 // "WAVE" in little-endian
    ...
    private struct DS64Chunk {
        ...
        var riffSize: UInt64 {
            return (UInt64(riffSizeHigh) << 32) | UInt64(riffSizeLow)
        }
        
        var dataSize: UInt64 {
            return (UInt64(dataSizeHigh) << 32) | UInt64(dataSizeLow)
        }
        
        var sampleCount: UInt64 {
            return (UInt64(sampleCountHigh) << 32) | UInt64(sampleCountLow)
        }
        ...
        init(data: Data, offset: Int) throws {
            ...
            self.sampleCountHigh = chunkData.withUnsafeBytes { $0.load(fromByteOffset: 24, as: UInt32.self) }
            self.sampleCountLow = chunkData.withUnsafeBytes { $0.load(fromByteOffset: 28, as: UInt32.self) }
    ...
    public static func isRF64File(at filePath: String) -> Bool {
        do {
            ...
            return header.riffChunkID == RF64_CHUNK_ID && 
    ...
    public static func parseRF64Info(at filePath: String) throws -> (dataSize: UInt64, sampleCount: UInt64, isValidRF64: Bool) {
        let fileURL = URL(fileURLWithPath: filePath)
        ...
        guard header.riffChunkID == RF64_CHUNK_ID,
              header.format == WAVE_FORMAT_ID else {
    ...
    public static func getRF64SupportStatus(for filePath: String) -> String {
        ...
        if testCoreAudioRF64Support(at: filePath) {
            ...
            do {
                ...
                return String(format: "RF64 file detected (%.2f GB data). CoreAudio does not support RF64 on this system. Consider converting to multiple smaller WAV files or using a different tool.", dataSizeGB)
            } catch {
                return "RF64 file detected but could not parse file information: \(error.localizedDescription)"
            }
        }
        #else
